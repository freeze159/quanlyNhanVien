...
nhanVienSV.layDSNV().then(function (res) {
    console.log(res.data);
    for (var i = 0; i < res.data.length; i++) {
        var maNV = res.data[i].MaNV;
        var ho = res.data[i].Ho;
        var ten = res.data[i].Ten;
        var ngayBatDau = res.data[i].NgayBatDau;
        var chucVu = res.data[i].ChucVu;

        var nhanVien = new NhanVien(ho, ten, maNV, ngayBatDau, chucVu);
        DSNV.push(nhanVien);
        taoBang(DSNV);
    }
})
...
function FormValidation() {
    var isValid = true;
    isValid &= KiemTraRong('ho', 'thongBaoHo', 'Vui lòng nhập họ') && KiemTraDinhDangChu('ho', 'thongBaoHo', 'Vui lòng nhập chữ');
    isValid &= KiemTraRong('ten', 'thongBaoTen', 'Vui lòng nhập tên') && KiemTraDinhDangChu('ho', 'thongBaoTen', 'Vui lòng nhập chữ');
    return isValid;
...
function xoaNhanVien(index) {
    Swal({
        ...
        if (result.value) {
            axios({
                method: 'DELETE',
                url: `https://5bd2959ac8f9e400130cb7e9.mockapi.io/api/nhanVien/${DSNV[index].MaNV}`
            })
                .then(function (res) {
                    console.log(res);
                    DSNV.splice(index, 1);
